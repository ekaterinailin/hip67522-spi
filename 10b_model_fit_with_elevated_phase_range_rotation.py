...
if __name__ == "__main__":
    ...
    tess_phases = np.loadtxt("results/tess_phases_rotation.txt")
    cheops_phases = np.loadtxt("results/cheops_phases_rotation.txt")
    flares = pd.read_csv("results/hip67522_flares_rotation.csv")
    ...
    # flare phases
    phases = flares.rot_phase.values
    ...
    # ----------------------------------------------------------------------------
    # MCMC CHAIN PLOTS -----------------------------------------------------------
    mod_samples = mod_sampler.get_chain(discard=ntrials//10*7)
    ...
    if diagnostics:
        ...
        plt.tight_layout()
        plt.savefig(f"plots/diagnostic/poisson_model/{nbinedges}_chain_modulated_rotation.png", dpi=300)
    ...
    if diagnostics:
        ...
        plt.savefig(f"plots/diagnostic/poisson_model/{nbinedges}_corner_modulated_rotation.png", dpi=300)
    ...
    mod_flat_samples = mod_flat_samples[mod_flat_samples[:, 1] > 0.3]
    ...
    if diagnostics:
        ...
        plt.tight_layout()
        plt.savefig(f"plots/diagnostic/poisson_model/{nbinedges}_chain_unmodulated_rotation.png", dpi=300)
    ...
    if diagnostics:
        ...
        plt.tight_layout()
        plt.savefig(f"plots/diagnostic/poisson_model/{nbinedges}_corner_unmodulated_rotation.png", dpi=300)
    ...
    if diagnostics:
        ...
        plt.tight_layout()
        plt.savefig(f"plots/diagnostic/poisson_model/{nbinedges}_best_median_fit_rotation.png", dpi=300)
    ...
    with open("results/bestfit_parameters_aic_rotation.txt", "a") as f:
