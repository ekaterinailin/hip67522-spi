...
if __name__ == "__main__":
    ...
    # use the same phase zero point for all sectors and CHEOPS for the rotation version
    sector_midpoints = [midpoint, midpoint, midpoint]
    tess_phases = get_tess_orbital_phases(period, [11, 38, 64], sector_midpoints) 

    # write phases to file
    np.savetxt("results/tess_phases_rotation.txt", tess_phases)
    np.savetxt("results/cheops_phases_rotation.txt", cheops_phases)

    # read hip 67522 flares
    flares = pd.read_csv("results/hip67522_flares.csv")

    flares["rot_phase"] = ((flares.t_peak_BJD - midpoint) % period) / period

    flares.to_csv("results/hip67522_flares_rotation.csv", index=False)
